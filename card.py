...
class Card(Square):
    ...
    def _advance_to_square(
        player, square_index, roll, state
    ...
    def _get_out_of_jail_free(player, state):
        return (
            GroupOfChanges()
        )  # 'jail free' card is handled by GameStateChange.draw_card()
    ...
    def _pay_building_fees_chance(player, state):
        return Card._pay_building_fees(
            player, state, CHANCE_PER_HOUSE_FEE, CHANCE_PER_HOTEL_FEE
        )
    ...
    def _advance_to_nearest_utility(player, state):
        ...
        nearest_utility = Card._nearest_to(
            player.position, [electric_company, water_works]
        )
        roll = Roll().value
    ...
    def _advance_to_nearest_railroad(player, state):
        ...
        nearest_railroad = Card._nearest_to(
            player.position,
            [
                reading_railroad,
    ...
    def _collect_100(
        player, state
    ):  # "xmas funds", "you inherit 100", "life insurance"
    ...
    def _pay_building_fees_community_chest(player, state):
        return Card._pay_building_fees(
            player, state, COMMUNITY_CHEST_PER_HOUSE_FEE, COMMUNITY_CHEST_PER_HOTEL_FEE
        )
