...
class GameStateChange(object):
    ...
    def combine(self, changes):  # TODO: Deprecated - remove combine()
        ...
        for change in changes:
            ...
            for (
                player,
                change_in_jail_free_count,
            ) in change._change_in_jail_free_count.items():
    ...
    def transfer_money(player_from, player_to, amount):
        return GameStateChange(
            change_in_cash={player_from: -amount, player_to: +amount},
            description=player_from.name
            + " paid "
            + str(amount)
            + " to "
            + player_to.name,
    ...
    def buy_property(prop, player, bank, mortgaged=False):
        ...
        if mortgaged:
            ...
            description += " in full"
        return GameStateChange(
            change_in_cash=transfer_money.change_in_cash,
    ...
    def change_position(player, new_position, bank, squares):
        ...
        if (
            player.position >= (INDEX[GO] - max_roll) % NUM_SQUARES
            and new_position < (INDEX[GO] - max_roll) % NUM_SQUARES
        ):
            # Player passed GO
            ...
            return GameStateChange(
                new_position={player: new_position}, description=description
            )
    ...
    def mortgage(prop, state):
        ...
        return GameStateChange(
            is_mortgaged={prop: True},
            change_in_cash={owner: +prop.price / 2, state.bank: -prop.price / 2},
    ...
    def build(prop, state):
        ...
        if prop.num_houses == NUM_HOUSES_BEFORE_HOTEL:
            # Build a hotel
            return GameStateChange(
                change_in_houses={prop: +1},
                change_in_cash={
                    owner: -prop.house_price,
                    state.bank: +prop.house_price,
                },
    ...
    def demolish(prop, state):
        ...
        if prop.has_hotel():
            ...
            return GameStateChange(
                change_in_houses={prop: -1},
                change_in_cash={
                    owner: +prop.house_price / 2,
                    state.bank: -prop.house_price / 2,
                },
                change_in_houses_remaining=-NUM_HOUSES_BEFORE_HOTEL,
                change_in_hotels_remaining=+1,
    ...
    def decrement_jail_card_count(player, deck):
        ...
        return GameStateChange(
            card_replaced={deck: LMBDA_GET_OUT_OF_JAIL_FREE},
            change_in_jail_free_count={player: -1},
    ...
    def send_to_jail(player):
        return GameStateChange(
            new_position={player: INDEX[JAIL]},
    ...
    def leave_jail(player):
        return GameStateChange(
            change_in_jail_moves={player: -player.jail_moves},
    ...
    def eliminate(player_eliminated, player_eliminator, state):
        ...
        for prop, num_houses in demolitions.items():
            house_money += (
                prop.house_price * 0.5 * -num_houses
            )  # num_houses is negative
        ...
        return GameStateChange(
            new_position={player_eliminated: -1},
            is_in_game={player_eliminated: False},
            change_in_cash={
                player_eliminated: -player_eliminated.cash,
                player_eliminator: +player_eliminated.cash + house_money,
            },
            removed_props={player_eliminated: player_eliminated.props},
            added_props={player_eliminator: player_eliminated.props},
