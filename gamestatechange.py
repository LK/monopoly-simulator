...
class GameStateChange(object):
  ...
  def buy_property(prop, player, bank, mortgaged=False):
    ...
    transfer_money = None
    if mortgaged:
      transfer_money = GameStateChange.transfer_money(player, bank, 0.5 * prop.price)
  ...
  def build(prop, state):
    ...
    if prop.num_houses == NUM_HOUSES_BEFORE_HOTEL:
      ...
      return GameStateChange(change_in_houses={ prop: +1 },
        change_in_cash={ owner: -prop.house_price, state.bank: +prop.house_price },
  ...
  def demolish(prop, state):
    ...
    if prop.has_hotel():
      # Demolish a hotel
      return GameStateChange(change_in_houses={ prop: -1 },
        change_in_cash={ owner: +prop.house_price / 2, state.bank: -prop.house_price / 2 },
        change_in_houses_remaining=-NUM_HOUSES_BEFORE_HOTEL,
        change_in_hotels_remaining=+1,
  ...
  def eliminate(player_eliminated, player_eliminator, state):
    ...
    is_mortgaged = { }
    change_in_jail_free_count = { player_eliminated: -player_eliminated.jail_free_count }
    if player_eliminator == state.bank:
      ...
      # cards
      change_in_jail_free_count[player_eliminator] = +player_eliminated.jail_free_count
