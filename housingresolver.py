...
class HousingResolver(object):
    ...
    def _is_hotel_shortage(self, were_hotels_demolished):
        if were_hotels_demolished:
            ...
            return (
                self._hotels_built
                > self._state.hotels_remaining + self._hotels_demolished
            )
    ...
    def _is_house_shortage(self, were_houses_demolished, were_hotels_built):
        if were_houses_demolished and were_hotels_built:
            ...
            if not were_houses_demolished and were_hotels_built:
                ...
                num_houses_to_become_available = (
            ...
            return (
                self._houses_built
                > self._state.houses_remaining + num_houses_to_become_available
            )
        ...
        for i in range(num_houses):
            ...
            while True:
                ...
                if highest_bid_for_round > highest_bid:
                    ...
                    highest_changes = highest_changes_for_round[
                        list(highest_changes_for_round.keys())[0]
                    ]

                if (
                    len(highest_changes_for_round) == 0
                    or list(players_in_auction.values()).count(True) <= 1
                ):
                    break
    ...
    def _auction_house_builds(num_houses, players):
        bid = lambda player, highest_bid, props, state: player.bid_house_builds(
            highest_bid, props, state
        )
        extract_bid = (
            lambda change, state: change.change_in_cash[state.bank]
            - list(change.changes_in_houses.keys())[0].house_price
        )
    ...
    def _auction_hotel_builds(num_hotels, players):
        bid = lambda player, highest_bid, props, state: player.bid_hotel_builds(
            highest_bid, props, state
        )
        extract_bid = (
            lambda change, state: change.change_in_cash[state.bank]
    ...
    def _auction_hotel_demolitions(num_houses, players):
        bid = lambda player, highest_bid, props, state: player.bid_hotel_demolitions(
            highest_bid, props, state
        )
        extract_bid = (
            lambda change, state: change.change_in_cash[state.bank]
            + list(change.changes_in_houses.keys())[0].house_price / 2
        )
    ...
    def _are_enough_houses_for_hotel_demolitions(self, were_houses_built):
        ...
        if were_houses_built:
            ...
            return (
                num_houses_needed <= self._state.houses_remaining + self._houses_built
            )
    ...
    def _settle_hotel_demolitions(self, were_houses_built):
        if self._are_enough_houses_for_hotel_demolitions(were_houses_built):
            ...
            if len(players_demolishing_hotels) > 1:
                ...
                original_hotel_demolitions = player_building_requests[
                    player
                ].hotel_demolitions
    ...
    def _resolve(self):
        ...
        if self._is_hotel_shortage(HousingResolver._BEFORE_HOTEL_DEMOLITIONS):
            ...
            if not self._is_house_shortage(
                HousingResolver._AFTER_HOUSE_DEMOLITIONS, HousingResolver._OVERRIDE_TRUE
            ):
                self._build_houses()
            else:
                # Auction houses
                result_of_auction = HousingResolver._auction_house_builds(
                    self._state.houses_remaining,
                    self._get_players_building_houses(),
                    self._state,
                )
                self._state.apply(result_of_auction)
            ...
            self._settle_hotel_demolitions(
                HousingResolver._AFTER_HOUSE_BUILDS
            ...
            if not self._is_hotel_shortage(HousingResolver._AFTER_HOTEL_DEMOLITIONS):
                ...
                result_of_auction = HousingResolver._auction_hotel_builds(
                    self._state.hotels_remaining,
                    self._get_players_building_hotels(),
                    self._state,
                )
                self._state.apply(result_of_auction)

        elif self._is_house_shortage(
            HousingResolver._AFTER_HOUSE_DEMOLITIONS,
            HousingResolver._BEFORE_HOTEL_BUILDS,
        ):
            # 2: Build hotels
            ...
            if not self._is_house_shortage(
                HousingResolver._AFTER_HOUSE_DEMOLITIONS,
                HousingResolver._AFTER_HOTEL_BUILDS,
            ):
                self._build_houses()
            else:
