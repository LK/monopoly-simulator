...
class Player(object):
    ...
    def copy(self):
        return Player(
            ...
            cash=self._cash,
            props=self._props,
            property_group_counts=self._property_group_counts,
            decision_maker=self._decision_maker,
            jail_free_count=self._jail_free_count,
            jail_moves=self._jail_moves,
    ...
    def is_property_group_complete(self, property_group):
        ...
        for prop in self.props:
            if prop.property_group == property_group:
                return (
                    prop.size_of_property_group
                    == self.property_group_counts[property_group]
                )
    ...
    def bid_house_builds(self, highest_bid, props_to_build_on, state):
        return self.decision_maker.bid_house_builds(
            self, highest_bid, props_to_build_on, state
        )

    def bid_hotel_builds(self, highest_bid, props_to_build_on, state):
        return self.decision_maker.bid_hotel_builds(
            self, highest_bid, props_to_build_on, state
        )

    def bid_hotel_demolitions(self, highest_bid, props_to_demolish_on, state):
        return self.decision_maker.bid_hotel_demolitions(
            self, highest_bid, props_to_demolish_on, state
        )
    ...
    def revise_hotel_demolitions(self, original_hotel_demolitions, state):
        return self.decision_maker.revise_hotel_demolitions(
            self, original_hotel_demolitions, state
        )
